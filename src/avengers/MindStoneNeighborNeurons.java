...
public class MindStoneNeighborNeurons {
    ...
    public static void main (String [] args) {
        ...
        for(int i = 0; i < n; i++) {
            nodes[i][0] = StdIn.readString();
        }
        // 'm' = mindstone, 'r' = regular node
        int m = StdIn.readInt();
        String[][] edges = new String[m][2];
        for(int i = 0; i < m; i++) {
            ...
            edges[i][1] = end;
            for(int j = 0; j < n; j++) {
                if(nodes[j][0].equals(start))
                    nodes[j][1] = "r";
            }
        }
        int mindStoneIndex = -1;
        for(int i = 0; i < n; i++) {
            if(nodes[i][1] == null) {
                nodes[i][1] = "m";
                mindStoneIndex = i;
        ...
        for(int i = 0; i < m; i++) {
            if(edges[i][1].equals(nodes[mindStoneIndex][0]))
                StdOut.print(edges[i][0] + '\n');
        }
